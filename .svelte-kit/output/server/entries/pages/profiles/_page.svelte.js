...
const Table = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="mt-10 pt-10 w-full max-w-xl p-12 mx-auto rounded-lg shadow-xl dark:bg-white/10 bg-white/30 ring-1 ring-gray-900/5 backdrop-blur-lg"><div class="flex items-center justify-between mb-4"><div class="space-y-1"><h2 class="text-xl font-semibold" data-svelte-h="svelte-1h6aat9">List of Users</h2> <p class="text-sm text-gray-500">Fetched ${escape(names.length)} users</p></div></div> <div class="divide-y divide-gray-900/5">${each(names, (user) => {
    return `<div class="flex items-center justify-between py-3"><div class="flex items-center space-x-4"><div class="flex"><p class="font-medium pt-1 leading-none">${escape(user.name)}</p> <p class="font-medium pl-5 text-gray-500 pt-0">${escape(user.email)}</p> </div></div> <form method="POST" action="/profiles?/delete"><input type="hidden" name="id" id="id"${add_attribute("value", user.id, 0)}> <button type="submit" data-svelte-h="svelte-1pobigz"><img class="w-4 float-right" src="./trash-can.svg" alt="delete"> </button></form> </div>`;
  })}</div></div>`;
});
